...
double binSVM::getKerlenPr(const std::vector<double>& _x, size_t number) {
...
std::pair<std::string, double> binSVM::getPairPredict(const std::vector<double>& _x) {
	const double v = finalF(_x);
	const std::string key = (v >= 0.0) ? labels.first : labels.second;
	const double w = 1.0 / (1 + std::exp(-std::abs(v)));
	return std::make_pair(key, w);
}

// предсказание метки для объекта _x
...
// предсказание меток для классифицируемых объектов
...
std::vector<size_t>& binSVM::getNums() {
	return nums;
}

std::vector<double>& binSVM::getA() {
	return a;
}

//
void binSVM::setNums(const std::vector<size_t>& i) {
	nums = i;
}

void binSVM::setA(const std::vector<double>& i) {
	a = i;
}
