...
std::vector<std::string> ovoSVM::tbb_predict(const std::vector<std::vector<double>>& _x) {
	std::vector<std::string> res(_x.size());
	tbb::parallel_for(
		tbb::blocked_range<size_t>(0, _x.size()),
...
std::string ovoSVM::balanced_predict(const std::vector<double>& _x) {
	if (binClassifiers.empty()) return "";
...
std::vector<std::string> ovoSVM::balanced_predict(const std::vector<std::vector<double>>& _x) {
	std::vector<std::string> res;
	res.reserve(_x.size());
	for (const auto &xi: _x) {
		res.push_back(balanced_predict(xi));
	}
	return res;
}

std::vector<std::string> ovoSVM::tbb_balanced_predict(const std::vector<std::vector<double>>& _x) {
	std::vector<std::string> res(_x.size());
	tbb::parallel_for(
		tbb::blocked_range<size_t>(0, _x.size()),
		[&](const tbb::blocked_range<size_t>& range) {
			for (size_t i = range.begin(); i != range.end(); i++) {
				res[i] = balanced_predict(_x[i]);
			}
		}
	);
	return res;
